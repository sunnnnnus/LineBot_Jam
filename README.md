...
倒垃圾做完了        → Bot 會把那筆待辦標記成完成
```

一則訊息裡列多件事(1. 2. 3. 條列、頓號或換行都可以)會一次全部收下,不會只吃到第一件。
...
3. **既有待辦做完了**(「倒垃圾好了」「第 2 件完成」「開會跟繳費都 done 了」)→ AI 以待辦清單上的編號呼叫 `complete_tasks`,直接把 `Status` 改成 `done`,不再多問一次確認(完成是可逆的,再講一次要做就好)。一次可以標記多筆;同一則訊息若同時又要新增,完成結果與新任務的確認問句會合併成一則回覆。
4. **不是新增意圖**(單純聊天、詢問待辦清單等)→ 直接文字回覆,不留記憶。

> 給 AI 的待辦清單帶著資料庫編號(最多 30 筆),`complete_tasks` 回來的編號一律再用 `LineUserId` 限縮查詢一次,模型就算給了別人的編號也改不到別人的資料。
...
使用者傳送 LINE 訊息
 ...
 有等待確認的提議,且回覆是「確定/取消」精確字眼?
   ...
   ▼是        ▼否(或沒有等待確認的提議)
直接處理    呼叫 AI(Groq)
(零延遲)   (帶 create_tasks / ask_clarification / confirm_task /
   │        cancel_task / complete_tasks 五個工具)
   │             │
   │      ┌──────┼──────────┬──────────────┬──────────┐
   │      ▼      ▼          ▼              ▼          ▼
   │  create_tasks ask_clarification confirm/cancel_task complete_tasks 純文字
   │      │           │              │              │   (聊天,不留記憶)
   │      ▼           ▼              ▼              ▼
   │  存為暫存提議   存對話上下文  真正寫入/清空 TASKS  標記 done
   │  回覆確認問句   回覆追問                      (可與新增合併回覆)
   └──────┴───────────────────────────┴──────────────┘
                    │
                    ▼
...
- [x] 一則訊息多筆待辦(`create_tasks` 改成吃 `tasks` 陣列——`gpt-oss-120b` 不支援平行 tool call,所以不靠多次呼叫,改成一次回傳整個陣列)
- [x] 口語化標記完成(`complete_tasks`:AI 依待辦編號把任務改成 `done`,可一次多筆;查詢一律以 `LineUserId` 限縮,避免模型給錯編號動到別人的資料)
- [x] 容器時區對齊 `Asia/Taipei`(時間欄位都是不帶時區的 `TIMESTAMP`、程式一律用 `DateTime.Now`,容器預設的 UTC 會讓「明天下午 6 點」算錯、提醒晚 8 小時發出;`Dockerfile` 已安裝 tzdata 並設 `ENV TZ`)
- [x] 確認逾時只作廢真的過期的那幾筆,其餘照常新增,並在回覆裡說明跳過了幾筆

### 待決定事項

- 群組裡建立的任務,提醒目前是私訊給建立者,不是推回群組
...
> **時區**:容器預設跑 UTC,但所有時間欄位都是不帶時區的 `TIMESTAMP`、程式一律用 `DateTime.Now`,不對齊時區會讓「明天下午 6 點」解析成 UTC 的明天、提醒也晚 8 小時發出。`Dockerfile` 裡裝了 tzdata 並設定 `ENV TZ=Asia/Taipei`(.NET 在 Linux 要有 tzdata 才解析得出時區,base image 不保證內建)。部署後可以直接問 Bot「現在幾點」驗證。
...
同一輪掃描中，同一使用者、同一提醒階段且到期時間相差不超過 30 分鐘的待辦會合併推播。30 分鐘以組內最早到期的事項為起點，避免相鄰事項一路串成跨度很大的群組；每則最多 10 件，超過會分則，並逐件顯示到期時間。不同使用者不會混在一起，也不會為了合併而提早發送尚未進入該提醒階段的事項，或延後已該發送的提醒。

可用 `Reminder__MergeWindowMinutes` 調整合併範圍，預設 30，設為 0 只合併完全相同時間，最大 1440。每件待辦仍各自保存提醒紀錄；整則推播失敗不會標記任何一件已送出。多實例掃描會鎖定並重新檢查紀錄，避免同時送出相同批次；推播成功後資料庫寫入失敗的既有風險仍存在。

提醒使用 🔔／🕒，新增確認與成功訊息使用 📝／✅；AI 的語氣指示為自然使用 1～2 個合適 emoji，嚴肅或災害問題保持克制，不在待辦內容中擅自新增 emoji。
